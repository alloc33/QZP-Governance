...
describe("qzl-labs-tests", () => {
  ...
  before(async () => {
    try {
      ...
      await tokenProgram.methods
        .createMintAccount({
          name: "Quantzilla Labs Token", // Name of the token.
          symbol: "QZL", // Symbol for the token.
          uri: "https://my-token-data.com/metadata.json", // URI pointing to token metadata.
          initialSupply: new anchor.BN(QZL_TOKEN_INITIAL_SUPPLY), // Initial supply of the token.
        })
    ...
    try {
      ...
      // Define the accounts involved in transferring QZL tokens from the mint to the voter.
      ...
      // Execute the token transfer of 10,000 QZL from admin to voter A.
      await tokenProgram.methods
      ...
      // Execute the token transfer of 10,000 QZL from admin to voter B.
      await tokenProgram.methods
        .transferTokens(new anchor.BN(10_000)) // Amount of QZL tokens to transfer.
      ...
      console.log("QZL tokens transferred to test voter.");
    } catch (error) {
   ...
   * Purpose: Ensure that user has enough QZL to vote
   */
  it("Insufficient tokens for voting fee should fail", async () => {
    ...
    // Create a user with no QZL tokens
    ...
    // (No token transfer from admin, so user has 0 QZL)
