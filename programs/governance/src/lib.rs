...
mod governance {
    ...
    ///
    /// **Parameters:**
    /// - `ctx`: Context containing the accounts required for initialization.
    /// - `token_mint`: The public key of the token mint associated with the governance token.
    /// - `token_program`: The public key of the SPL Token program.
    /// - `init_vote_fee`: The initial fee required to cast a vote.
    pub fn initialize(
        ctx: Context<Admin>,
        token_mint: Pubkey,
    ...
    pub struct VoteManager {
        ...
        pub tk_mint: Pubkey,    // Token mint for governance token (QZL).
